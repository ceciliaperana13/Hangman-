...
def main_menu(screen, clock, options_manager):
    ...
    while True:
        ...
        # Swing animation for the gallows
        time += 1
...
def page_options(screen, clock, options_manager):
    ...
    switch_full_screen = buttonSwitch(
        400, 280, "Fullscreen:", options_manager.full_screen
    )
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if btn_return.for_clic(pos):
                    # Save options and return to menu
        ...
        if category == "systeme":
            title_cat = font.render("Screen and Display", True, WHITE)
            screen.blit(title_cat, (80, 160))
            Selector_res.draw(screen)
            switch_full_screen.draw(screen)
        else:
            title_cat = font.render("Audio Settings", True, WHITE)
            screen.blit(title_cat, (80, 160))
